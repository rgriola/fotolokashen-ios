...
struct LocationPhoto: Codable, Equatable, Hashable {
    ...
    let isPrimary: Bool?
    /// Pre-computed ImageKit size variants (Phase 0/1a). Optional for backward
    /// compatibility with older API responses.
    let sizes: PhotoSizes?

    // Custom Codable to keep `sizes` optional without breaking existing decode.
    enum CodingKeys: String, CodingKey {
        case id, imagekitFilePath, isPrimary, sizes
    }

    // Convenience URL accessors — prefer server-provided `sizes`, fall back to
    // client-side `ImageKitURL` building.
