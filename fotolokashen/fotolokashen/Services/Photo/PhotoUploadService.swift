...
class PhotoUploadService: ObservableObject {
    ...
    func uploadPhoto(
        ...
        // Compress and delegate to the data-based path so both entry points
        // share a single network/association implementation.
        guard let compressedData = ImageCompressor.compress(image) else {
            throw PhotoUploadError.compressionFailed
        }
        return try await uploadCompressedPhoto(
            data: compressedData,
            locationId: locationId,
            location: location,
            caption: caption,
    ...
    /// Phase 1d — Upload an already-compressed photo (skip-recompression path).
    ///
    /// `PhotoUploadQueue` calls this directly so it doesn't have to decode
    /// `Data → UIImage → re-compress` for every job. The compressed bytes
    /// are sent verbatim through the secure multipart endpoint.
    func uploadCompressedPhoto(
        ...
        do {
            ...
            if config.enableDebugLogging {
                print("[PhotoUpload] Skip-recompression upload, size=\(compressedData.count / 1024)KB")
            }
            #endif
            ...
            let secureResponse = try await uploadSecurely(
                data: compressedData,
                filename: filename ?? "photo_\(Int(Date().timeIntervalSince1970)).jpg",
                uploadType: "location",
                metadata: metadata
            )
