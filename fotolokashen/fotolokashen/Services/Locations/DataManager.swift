...
class DataManager: ObservableObject {
    ...
    private init() {
        do {
            ...
            if ConfigLoader.shared.enableDebugLogging {
                dlog("DataManager", "SwiftData initialized successfully")
            }
    ...
    func queuePhoto(_ offlinePhoto: OfflinePhoto) throws {
        ...
        if ConfigLoader.shared.enableDebugLogging {
            dlog("DataManager", "Photo queued for upload. Queue size: \(pendingUploads)")
        }
    }
    ...
    func removeFromQueue(_ photo: OfflinePhoto) throws {
        ...
        if ConfigLoader.shared.enableDebugLogging {
            dlog("DataManager", "Photo removed from queue. Remaining: \(pendingUploads)")
        }
    }
    ...
    private func updatePendingCount() async {
        do {
            ...
            if ConfigLoader.shared.enableDebugLogging {
                dlog("DataManager", "Error fetching pending count: \(error)")
            }
        }
    }
    ...
    func clearCache() throws {
        ...
        if ConfigLoader.shared.enableDebugLogging {
            dlog("DataManager", "Cache cleared")
        }
    }
    ...
    func clearQueue() throws {
        ...
        if ConfigLoader.shared.enableDebugLogging {
            dlog("DataManager", "Upload queue cleared")
        }
    }
}
