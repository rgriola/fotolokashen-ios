    ...
    func syncAll() async {
        guard networkMonitor.isConnected else {
            if config.enableDebugLogging {
                dlog("Sync", "Skipping sync - offline")
            }
            return
        }
        ...
        guard KeychainService.shared.getAccessToken() != nil else {
            if config.enableDebugLogging {
                dlog("Sync", "Skipping sync - no auth token")
            }
            return
        }
    ...
    func syncLocationsFromAPI() async {
        ...
        do {
            ...
            if !LocationStore.shared.locations.isEmpty {
                ...
                if config.enableDebugLogging {
                    dlog("Sync", "Reusing \(locations.count) locations from LocationStore (skipping duplicate API call)")
                }
            } else {
                if config.enableDebugLogging {
                    dlog("Sync", "Fetching locations from API...")
                }
                ...
                if config.enableDebugLogging {
                    dlog("Sync", "LocationStore updated with \(locations.count) locations")
                }
            }
            ...
            if config.enableDebugLogging {
                dlog("Sync", "\(locations.count) locations saved to cache")
            }

        } catch {
            if config.enableDebugLogging {
                dlog("Sync", "Location sync error: \(error)")
            }
        }
    }
    ...
    func syncPhotosToAPI() async {
        ...
        do {
            ...
            if queuedPhotos.isEmpty {
                if config.enableDebugLogging {
                    dlog("Sync", "No photos to upload")
                }
                return
            }
            
            if config.enableDebugLogging {
                dlog("Sync", "Uploading \(queuedPhotos.count) queued photos...")
            }
            ...
            for (index, offlinePhoto) in queuedPhotos.enumerated() {
                ...
                guard offlinePhoto.shouldRetry else {
                    if config.enableDebugLogging {
                        dlog("Sync", "Skipping photo \(offlinePhoto.clientId) - exceeded retry limit")
                    }
                    failedCount += 1
                    continue
                }
                ...
                do {
                    ...
                    if config.enableDebugLogging {
                        dlog("Sync", "Uploaded photo \(index + 1)/\(queuedPhotos.count)")
                    }
                    ...
                    if config.enableDebugLogging {
                        dlog("Sync", "Failed to upload photo: \(error)")
                        dlog("Sync", "Retry count: \(offlinePhoto.retryCount)/3")
                    }
                }
            }
            
            if config.enableDebugLogging {
                dlog("Sync", "Photo upload complete: \(uploadedCount) succeeded, \(failedCount) failed")
            }
            
        } catch {
            if config.enableDebugLogging {
                dlog("Sync", "Photo sync error: \(error)")
            }
